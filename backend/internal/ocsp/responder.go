...
func (r *Responder) loadClassicalIssuerKey(keyID string) error {
	...
	privateKey, err := pq.ParsePrivateKey(keyData)
	if err != nil {
...
func (r *Responder) GetStatistics() map[string]interface{} {
	...
	pqQuery := `SELECT COUNT(*) FROM certificates WHERE algorithms LIKE '%dilithium%' OR algorithms LIKE '%falcon%' OR algorithms LIKE '%sphincs%' OR algorithms LIKE '%multi-pqc%'`
